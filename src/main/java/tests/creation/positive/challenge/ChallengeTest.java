...
public class ChallengeTest extends BaseTest {
    ...
    @Test(priority = 1, description = "Pengguna membuka halaman Challenge dan membuat Public Challenge")
    @TestInfo(
    ...
    @Test(priority = 2, description = "Pengguna membuka halaman Challenge dan membuat Private Challenge")
    @TestInfo(
    ...
    public void testReplaceBannerOnly() {
        ...
        boolean isPosterUploaded = challengePage.areElementsDisplayed("Gambar berhasil diupload"); // Verifikasi pesan sukses upload
        if (isPosterUploaded) {
            TestListener.getTest().pass("SUCCESS: Second Poster successfully replaced the first one!");
        } else {
            logFail("FAILURE: Second Poster was not uploaded properly.");
        }
    ...
    public void testEditChallengeReplaceName() {
        ...
        // 8. Cleanup
        challengePage.navigateBackToDashboardSafe();
        challengePage.navigateToDetailChallenge(newChallengeName, false);
        challengePage.areElementsDisplayed(newChallengeName);
        challengePage.navigateBackToDashboardSafe();
        challengePage.navigateToBeranda(false);
        challengePage.areElementsDisplayed(newChallengeName);
        // challengePage.tapButtonByTextOrId(uniqueName, uniqueName);
    ...
    public void testShareChallengeToWhatsAppPublic() {
        ...
        String targetContact = "(Anda)"; 
        
        if (TestListener.getTest() != null) {
            TestListener.getTest().info("Action: Memulai proses Share Challenge ke WhatsApp target: " + targetContact);
        }

        String challengeName = "Lari Publik";
        String expectedMessage = "Hai, bergabunglah bersama saya dengan challenge lari yang saya buat! " + challengeName;
        // 2. Eksekusi fungsi Share
        // Fungsi ini sudah mencakup navigasi ke Kelola Challenge -> Klik Share -> Pilih WA -> Pilih Kontak -> Send
        challengePage.shareChallengeToWhatsApp(targetContact, challengeName, expectedMessage, false);
        
        if (TestListener.getTest() != null) {
            TestListener.getTest().info("Action: Proses share selesai. Mencoba kembali ke aplikasi utama...");
        }
        
        boolean isMessageDisplayed = challengePage.areElementsDisplayed(expectedMessage);

        if (isMessageDisplayed) {
            TestListener.getTest().pass("Share berhasil.");
        } else {
            logFail("Pesan share tidak muncul.");
        }
    ...
    public void testShareChallengeToWhatsAppPrivate() {
        ...
        String challengeName = "Lari Privat";
        String expectedMessage = "Hai, bergabunglah bersama saya dengan challenge lari yang saya buat! " + challengeName;
        // 2. Eksekusi fungsi Share
        // Fungsi ini sudah mencakup navigasi ke Kelola Challenge -> Klik Share -> Pilih WA -> Pilih Kontak -> Send
        challengePage.shareChallengeToWhatsApp(targetContact, challengeName, expectedMessage, false);
